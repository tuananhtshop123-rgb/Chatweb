...
const MAP_SIZE = 2400; 
const ISLAND_MIN = 400, ISLAND_MAX = 2000; 
...
io.on('connection', (socket) => {
    ...
    socket.on('join', (data, callback) => {
        ...
        // FIX LỖI ĐEN MÀN HÌNH: Phải gửi thông tin cá nhân ngay lập tức cho client
        socket.emit('syncPlayer', players[socket.id]);
    ...
    socket.on('addStat', (type) => {
        ...
        if (p && p.statPoints > 0) {
            ...
            if (type === 'lifesteal') p.stats.lifesteal += 1;
    ...
    socket.on('hitMonster', (data) => {
        ...
        if (p && m && !p.isDead && m.hp > 0) {
            let dmg = Math.floor(p.stats.atk * data.multiplier); 
            m.hp -= dmg;
            m.targetId = p.id; 
